...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const { body, status } = await safeDesktopResponse(await desktopMeetingFetch(
      `/internal/desktop/meetings/${access.meetingId}/status`,
      access.ownerPhone,
    ));
    return NextResponse.json(body, { status });
  } catch (error) {
    return NextResponse.json({ ok: false, error: "Meeting status is unavailable." }, { status: Number((error as { status?: number }).status) || 502 });
  }
}
